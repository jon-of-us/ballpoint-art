...
def main(path):
    ...
    f = interpolate.RectBivariateSpline(
        np.arange(z.shape[0]), np.arange(z.shape[1]), z * zScale
    )
    ...
    positions.append((by1, -bx1, z))
    points_bx0 = list(
        sorted([(x, y) for x, y in points if abs(x - bx0) < 1e-9], key=lambda p: p[1])
    )
    points_by0 = list(
        sorted([(x, y) for x, y in points if abs(y - by0) < 1e-9], key=lambda p: p[0])
    )
    points_bx1 = list(
        sorted([(x, y) for x, y in points if abs(x - bx1) < 1e-9], key=lambda p: p[1])
    )
    points_by1 = list(
        sorted([(x, y) for x, y in points if abs(y - by1) < 1e-9], key=lambda p: p[0])
    )
...
if __name__ == "__main__":
    # main(sys.argv[1])
    main("./output/Screenshot 2023-12-05 213522.png")
